...
/**
 * 新消息提醒class this class is subject to be inherited and implement the relative APIs
 */
public class EaseNotifier {
    ...
    private EaseSettingsProvider settingsProvider = new DefaultSettingsProvider();
    Ringtone ringtone = null;

    protected final static String[] msg_eng = {
            "sent a message", "sent a picture", "sent a voice",
            "sent location message", "sent a video", "sent a file", "%1 contacts sent %2 messages"
    };
    protected final static String[] msg_ch = {
            "发来一条消息", "发来一张图片", "发来一段语音", "发来位置信息", "发来一个视频", "发来一个文件",
            "%1个联系人发来%2条消息"
    };
    ...
    public synchronized void onNewMesg(List<EMMessage> messages) {
        if (EMClient.getInstance().chatManager()
                .isSlientMessage(messages.get(messages.size() - 1))) {
            return;
        }
